...
public class DataFlow {
   ...
   * as follows:
   * 
   * java randoop.experiments.DataFlow <em>subject-program-string</em>-
   * <em>suffix</em>
   * 
   ...
   * 
   * + The file <em>subject-program-string</em>-<em>suffix</em>.ser exists and
   * contains the serialized results of a run of Randoop on classes in the
   * subject program.
   * 
   * The command reads in the property file and uses it to create the
   * appropriate classpath. It invokes randoop.main.DataFlow with the given
   * classpath and passes the serialized file as input.
   */
  public static void main(String[] args) throws IOException {
    ...
    if (split.length != 2)
      throw new IllegalArgumentException("Invalid experiment string.");
    ...
    int retval = Command.exec(df.toArray(new String[0]), System.out, err, "", false, Integer.MAX_VALUE, null);
    if (retval != 0) {
