...
/**
 * A Predicate that implements the or-else operator on two predicates.
 * The or-else operator is a short-circuited or, which only tests the
 * second predicate if the first returns false.
 *
 * @param <T>  the type of object tested by the predicates
 */
public class OrPredicate<T> extends DefaultPredicate<T> {
  ...
  /**
   * Constructs a predicate that will test the first predicate 
   * and then the second.
   * 
  ...
  /**
   * {@inheritDoc}
   * Return the short-circuited or of the two predicates for the value t
