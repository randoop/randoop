...
/**
 * Predicate that implements the and-also operator on a pair of predicates.
 * The and-also operator is a short-circuited and operator, which only tests
 * the second predicate if the first predicate is true.
 *
...
public class AndPredicate<T> extends DefaultPredicate<T> {
  ...
  /**
   * Creates a Predicate that performs the and-also operator: testing the
   * first predicate, and then the second.
   * 
  ...
  /**
   * {@inheritDoc}
   * Return the short-circuited and of the two predicates for the value t
