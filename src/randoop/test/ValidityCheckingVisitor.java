 ...
 * {@code ExceptionPredicate}. Also, either ignores or reports flaky test
 * sequences --- an input sequence that throws an exception in a longer test
 * sequence, despite having run normally by itself. Ignores flaky sequences are
 * classified as invalid. Flaky occurrences of {@code OutOfMemoryError} are
...
public class ValidityCheckingVisitor implements TestCheckGenerator {
  ...
  /**
   * The flag to determine whether to report flaky tests by throwing an
   * exception
   */
   ...
   *          a flag indicating whether to report flaky tests by throwing an
   *          exception
   */
  ...
  /**
   * {@inheritDoc} Checks validity of a test sequence and creates a
   * {@code InvalidChecks} object containing checks for any invalid exceptions
   * encountered. Exceptions are classified by the {@code ExceptionPredicate},
