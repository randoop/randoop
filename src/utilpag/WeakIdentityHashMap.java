...
public class WeakIdentityHashMap<K,V>
    ...
    public WeakIdentityHashMap(int initialCapacity, float loadFactor) {
        ...
        while (capacity < initialCapacity)
            capacity <<= 1;
        table = new Entry[capacity]; // unchecked cast
    ...
    public WeakIdentityHashMap() {
        ...
        table = new Entry[DEFAULT_INITIAL_CAPACITY]; // unchecked cast
    }
    ...
    /**
     * Variable representing null keys inside tables.
     */
    ...
    void resize(int newCapacity) {
        ...
        Entry<K,V>[] newTable = new Entry[newCapacity]; // unchecked cast
    ...
    /**
     * Special-case code for containsVariable with null argument
     */
    private boolean containsNullVariable() {
	Entry<K,V>[] tab = getTable();
        for (int i = tab.length ; i-- > 0 ;)
            for (Entry e = tab[i] ; e != null ; e = e.next)
                if (e.value==null)
    ...
    private static class Entry<K,V> extends WeakReference<K> implements Map.Entry<K,V> {
        ...
        public V setValue(V newVariable) {
	    V oldVariable = value;
            value = newVariable;
            return oldVariable;
        }
    ...
    private class VariableIterator extends HashIterator<V> {
        public V next() {
    ...
    public Collection<V> values() {
        ...
        return (vs != null ?  vs : (our_values = new Variables()));
    }

    private class Variables extends AbstractCollection<V> {
        @Override
