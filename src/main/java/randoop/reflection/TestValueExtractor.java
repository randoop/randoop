...
public class TestValueExtractor implements ClassVisitor {
    ...
    private TestValueExtractor() {
        this.valueSequences = new LinkedHashSet<>();
    }

  /**
   * Returns the set of value sequences collected from the fields of visited classes.
   *
   * @return the set of value sequences collected by this visitor
   */
  public Set<Sequence> getValueSequences() { return valueSequences; }
    ...
    public void visit(Field f) {
        if (f.getAnnotation(TestValue.class) != null) {
            if (!Modifier.isStatic(f.getModifiers())) {
                String msg = "RANDOOP ANNOTATION ERROR: Expected @TestValue-annotated field "
                        + f.getName()
                        + " in class "
                        + f.getDeclaringClass()
