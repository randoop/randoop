...
public class ReturnCondition extends Condition {
  ...
  /**
   * Creates a {@link ReturnCondition} that calls the given condition method.
   *
   ...
   */
  ReturnCondition(Method conditionMethod, String comment, String conditionText) {
    super(conditionMethod, comment, conditionText);
  }

  /**
   * Returns the {@link ReturnCondition} that checks the condition with the given argument values as
   * the pre-state.
   *
   ...
   * @return the {@link ReturnCondition} with the pre-state set
   */
  ReturnCondition addPrestate(Object[] args) {
    return this;
  }
}
