...
public class ConstantMiningWrapper {
  ...
  /**
   * This class is a wrapper for the ConstantMining. The reason why the fields are enumerated for
   * each literal level instead of having one generic one is that we do not wish to introduce
   * generic constants to ComponentManager. If we finally decide to not have this wrapper, the
   * migration would be easy enough.
   */
