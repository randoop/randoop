...
public class GrtImpurity {
   ...
   *
   * <p>This is the main entry point for the GRT Impurity.
   * This method will append additional fuzzing statements to the input sequence to fuzz the input
   * object. This will generate more object states and potentially improve coverage for the unit
   * test generated by Randoop by triggering more branches.
   *
   ...
   *       Fuzzed using a Gaussian distribution with the original value as the mean (mu). The
   *       fuzzed value is generated as mu + N(0, GAUSSIAN_STD). For short values, if the fuzzed
   *       value exceeds the range, it is truncated. (truncatedShort = -32768 + fuzzedInt % 32768).
   ...
   *
   * <p>Precondition: The input sequence is not null.
   *
   ...
   * @throws RandoopBug if an unexpected exception occurs
   */
   ...
   * operation.
   * Precondition: The input statements are the last n statements in the sequence, where
  ...
  /**
   * Create a statement representing a Gaussian sampling result and append it to the given
   * sequence. This is used for fuzzing primitive numbers, being added to the original input
   * value (the mean).
   *
  ...
  private static Sequence appendGaussianSampleSequence(Sequence sequence, Class<?> cls) {
    ...
    temp.add(gaussianSampleSequence);
    return Sequence.concatenate(temp);
  }
