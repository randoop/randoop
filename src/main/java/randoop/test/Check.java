 ...
 * </ul>
 *
 * The visitor classes decorate a sequence with {@code Check} objects. A {@code Check}
 ...
 * m()}" is executed might emit the assertion code "{@code assertNotNull(x);}", and would
 * do so after the statement is printed. As a second example, a check for checking that an expected
 * exception is thrown by a statement would need to emit something like "{@code try {}" before
