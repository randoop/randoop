 ...
 * the oracle expects that every object contract holds. Any sequence of method
 * calls that leads to a failing contract is outputted by Randoop as an
 * error-revealing test case.
 * <p>
 *
...
public interface ObjectContract extends Serializable {
   ...
   * <p>
   * When calling this method during execution of a test, Randoop guarantees
   * that <code>objects</code> does not contain any <code>null</code> objects,
   * and that <code>objects.length == getArity()</code>.
   * <p>
   ...
   * @throws {@code Throwable} if an exception is thrown in evaluation 
   */
  boolean evaluate(Object... objects) throws Throwable;

  /**
   * Communicates to Randoop how to interpret exceptional behavior from the
   * <code>evaluate</code> method.
   * <p>
   * If this method returns <code>true</code>, Randoop will interpret an
   * exception that escapes during evaluation as a failure of the contract.
   * <p>
