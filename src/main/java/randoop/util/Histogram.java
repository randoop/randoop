...
public final class Histogram<T> {
  ...
  public void addToCount(T t, int i) {
    ...
    if (!fMap.containsKey(t)) fMap.put(t, i);
    else fMap.put(t, getCount(t) + i);
  }
  ...
  /**
   * This works only when keys are comparable. (In general, the keys are not
  ...
  private String entriesToString(boolean append_percent, Comparator<Map.Entry<T, Integer>> c) {
    ...
    if (fName != null) sb.append("Histogram:").append(fName).append(Globals.lineSep);
    int size = getSize();
    ...
    for (Iterator<Map.Entry<T, Integer>> iter = entries.iterator(); iter.hasNext(); ) {
      ...
      if (append_percent) sb.append(UtilMDE.rpad(createPercentString(size, e), 8));
  ...
  public int getCount(T t) {
    if (fMap.containsKey(t)) return fMap.get(t);
    else return 0;
  }
