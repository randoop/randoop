 ...
 * particular offset. When a unit test is run as a regression test, it should
 * have the same behavior as it did previously, and the Check objects represent
 * that previous, expected behavior. Some examples of Checks are:
 * <ul>
 ...
 * <code>Check</code> object is inserted as a decoration on a specific index of
 ...
 * "<code>assertNotNull(x);</code>", and would do so after the statement is
 * printed. As a second example, a check for checking that an expected exception
