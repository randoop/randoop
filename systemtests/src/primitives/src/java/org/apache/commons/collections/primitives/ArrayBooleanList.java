...
public class ArrayBooleanList extends RandomAccessBooleanList
     ...
     * @param that the non-<code>null</code> collection of <code>boolean</code>s
     *        to add
     ...
     * Any subsequent elements are shifted to the left, subtracting one from
     * their indices.  Returns the element that was removed.
     * 
    ...
    private final void checkRange(int index) {
        if(index < 0 || index >= _size) {
            throw new IndexOutOfBoundsException(
                    "Should be at least 0 and less than "
                    + _size + ", found " + index);
        }
    }
    ...
    private final void checkRangeIncludingEndpoint(int index) {
        if(index < 0 || index > _size) {
            throw new IndexOutOfBoundsException(
                    "Should be at least 0 and at most " + _size
                    + ", found " + index);
        }
    }
