...
public class ConcreteTypeTest {
  ...
  public void testForClass() {
    ...
    assertEquals("builds array type correctly", new ConcreteArrayType(new ConcreteSimpleType(String.class)), arrayType);
    
    ConcreteType rawClassType = ConcreteType.forClass(ArrayList.class);
    assertEquals("builds raw class type correctly", new ConcreteSimpleType(ArrayList.class), rawClassType);
        
    try {
      ConcreteType badParameterizedType = ConcreteType.forClass(ArrayList.class, new ConcreteSimpleType(int.class));
      fail("illegal argument exception expected");
    } catch (IllegalArgumentException e) {
      assertEquals("exception message", "type arguments may not be primitive (found: int)", e.getMessage());
    }
    
    ConcreteType boundedPType = ConcreteType.forClass(BoxClass.class, new ConcreteSimpleType(String.class));
